...
{
    device_handle_t  SNAI_485dev_handle[ALL_DEVICE_COUNT+1];
    int              Parameter_count[ALL_DEVICE_COUNT+1];
    int              Parameter_ptr[ALL_DEVICE_COUNT+1];
    bool             SNAI_device_ready[ALL_DEVICE_COUNT+1];
    bool             SNAI_DEVICE_EXIST[ALL_DEVICE_COUNT+1];
		unsigned char		 SNAI_485dev_ADDR[ALL_DEVICE_COUNT+1];
...
//假设读取0-5数据地址=则长度为6个字节 6作为下一次读取地址起始位OK
            if (((msg_type != SNAI_MSG_TYPE_DATA) && (msg_type != 0x11)) || (msg_len < SNAI_MSG_MIN_LEN))
...
void SNAI_DEVICE_RS485_ADDR_HANDLE(device_handle_t SNAI_handle,char *RS485_ADDR)
{
    const char *dat[ALL_DEVICE_COUNT+1]={
"0000",
"0001",
"0002",
"0003",
